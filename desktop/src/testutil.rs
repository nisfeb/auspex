...
//!
//! Everything the desktop shell talks to over HTTP (the bridge's upstream,
//! the login POST, the connection probe) takes a base URL, so a stub is
//! enough to assert what we actually send and what we do with the answer.
//! No ship, no network.
...
impl Stub {
    ...
    pub fn new(reply: impl Fn(&Seen) -> (u16, String) + Send + Sync + 'static) -> Stub {
        Stub::headed(move |s| {
            let (status, body) = reply(s);
            (status, Vec::new(), body)
        })
    }

    /// Answer with headers of the stub's choosing.
    ///
    /// The login route's entire payload is a `set-cookie`, so a stub that can
    /// only pick a status and a body cannot stand in for it, and the connect
    /// flow's one interesting failure — a 200 with no cookie on it — would be
    /// untestable.
    pub fn headed(
        reply: impl Fn(&Seen) -> (u16, Vec<(String, String)>, String) + Send + Sync + 'static,
    ) -> Stub {
        ...
        std::thread::spawn(move || {
            for conn in listener.incoming().flatten() {
                ...
                std::thread::spawn(move || {
                    ...
                    let (status, headers, body) = reply(&req);
                    ...
                    let _ = write!(
                        c,
                        "HTTP/1.1 {status} {}\r\n{loc}{extra}Content-Type: text/plain\r\n\
                         Content-Length: {}\r\nConnection: close\r\n\r\n",
                        reason(status),
                        body.len()
                    );
