...
```
desk/
  sys.kelvin                  [%zuse 409] and [%zuse 408]
  desk.bill                   ~[%urmail]
...
- [ ] **Step 6: Prove the trust boundary live**

`lib/test-agent.hoon` does not compile under zuse 408, which is what the dev
ships run: `-build-file /=urmail=/lib/test-agent/hoon` fails with `nest-fail`,
`-need.?(%~ [i=/ t=it(/)])` against `-have.[it(/) @p /]` — a kernel-level type
difference between 408 and 409 inside a 453-line vendored harness. Every copy
on disk is byte-identical to it; `feb/mcp` carries one only because nothing
imports it, so it is never built. There is therefore **no agent-test harness
available on this desk**, and `desk/tests/app/` must stay empty — clay builds
everything under `tests/`, so a file that cannot build makes the whole suite
report `ok=%.n` and trains everyone to ignore red.

The trust boundary is proven live instead. Cross-ship testing alone does not
cover it: in every `~wex`/`~feb` exchange both ships are participants, so the
property that `+receive` accepts a chain from a courier who is **not** a
participant has to be exercised deliberately.

On `~wex`, build a chain between two ships that are neither `~wex` nor `~feb`
and poke it in:

```
=urm -build-file /=urmail=/lib/urmail/hoon
=u [~sampel-palnet 1 (sy ~[~palnet-sampel]) 'boundary' 'from a stranger' ~2026.1.1 ~]
=m [u (sign-with:urm (fake-ring:urm ~sampel-palnet) (digest:urm u))]
:urmail &urmail-chain ~[m]
```

Read state with `:urmail &dbug [%state '']` — `+dbug` is broken ship-wide on
these ships, unrelated to this desk.

Expected: the thread is stored and `~sampel-palnet`'s message reads
`%verified`, even though `~wex` is neither sender nor recipient. A participant
check in `+receive` would have rejected this poke, so a clean accept is the
proof.

Then poke a chain longer than `max-chain` and confirm it is **rejected**, not
truncated, and that state is unchanged.

Record both verbatim. The trust boundary is live-verified, not unit-tested;
restoring the unit test requires a 408-compatible `test-agent.hoon`.
...
Expected: `ok=%.y`, nineteen `OK` lines, no `FAILED`.
...
```
.^(json %gx /(scot %p our)/urmail/(scot %da now)/inbox/json)
```
...
**Interfaces:**
- Consumes: the inbox and thread scries, the `/updates` subscription, and the `%urmail-action` poke from Task 6. Over HTTP the scry paths are `/inbox` and `/thread/<id>` — Eyre's `/~/scry/` endpoint supplies the `%x` care, so the leading `/x` that `on-peek` matches must not appear in the client path.
...
// scry() builds `/~/scry/{app}{path}.json`, and Eyre's /~/scry/ endpoint
// already implies care %x — a path starting with /x double-prepends it and
// 404s. Verified against ~wex: /~/scry/urmail/x/inbox.json 404s,
// /~/scry/urmail/inbox.json returns the inbox. Omit the leading /x here,
// even though on-peek matches [%x %inbox ~].
export const inbox = () =>
