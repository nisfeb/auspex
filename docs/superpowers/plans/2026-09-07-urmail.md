...
- Produces, all in `lib/urmail.hoon`:
  ...
  - `+verify-chain` `|= [keys=(map [ship @ud] (unit pass)) c=chain:sur] -> (list [msg-id:sur verdict:sur])` — keyed on `[ship life]`, so a signature stays verifiable across key rotation
...
Expected: twelve `OK` lines, no `FAILED`, no `CRASHED`.
...
++  send
  ...
  =.  verdicts   (~(put by verdicts) (id:urmail u) %verified)
  :_  this
...
++  receive
  ...
  =.  verdicts
    %+  roll  vs
    |=  [[i=msg-id:sur v=verdict:sur] acc=_verdicts]
    ?:  (~(has by acc) i)  acc
    (~(put by acc) i v)
  =.  inbox  [rid (skip inbox |=(t=thread-id:sur =(t rid)))]
  `this
...
Expected: the twelve library tests plus two agent tests, all `OK`.
...
++  msg-json
  ...
  :~  ['id' [%s (scot %uv i)]]
      ...
      ['verdict' [%s (~(gut by verdicts) i %unverified)]]
