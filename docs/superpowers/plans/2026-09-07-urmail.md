...
- Produces: the `%urmail` agent accepting an `%urmail-action` poke. Scry wrappers `+our-ring`, `+peer-pass`, `+key-map` live in `app/urmail.hoon` and are used by Task 5.
...
Add a `%urmail-chain` branch to `+on-poke`, above the closing `==`:

```hoon
      %urmail-chain
    (receive !<(chain:sur vase))
```
...
Add to the agent core, above `+on-peek`:
