...
- `sys.kelvin` is `[%zuse 409]`. Match the running dev ships; do not bump.
...
::
::    The map is keyed on [ship life], not ship alone: `life` travels inside
::    the message so a signature outlives key rotation, and a ship-only map
::    would label every pre-rotation message %forged - an accusation of
::    forgery where the honest answer is "I do not have that key".
::
++  verify-chain
...
Expected: eighteen `OK` lines, no `FAILED`, no `CRASHED`.
...
++  receive
  ...
  =.  threads
    %+  ~(put by threads)  rid
    [new (participants:urmail new) (last-sent:urmail new)]
  ::  identical ids imply identical bytes, so a verdict we already hold is
  ::  never overwritten by an incoming copy
  =.  verdicts
    %+  roll  vs
...
Expected: the eighteen library tests plus two agent tests, all `OK`.
